...
## Milestone: full backend chain live, end to end

Service definition, service binding (`ZRAP_MT_UI_SRVB`, OData V4 - UI,
published via `/IWFND/V4_ADMIN` → Publish Service Groups), and the
implementation class are all active. First `MigrationWorkspace` create
via the Fiori Elements preview is the live end-to-end proof: CDS
projection → behavior validation/field rules → number range → persistence,
all in one round trip.

**Root cause of the missing Create button, for the record:** the
behavior definition declared `managed implementation in class
zbp_i_rap_mt_hdr unique;`, but the class itself had never been generated
— activating the *behavior definition* text doesn't require the
*implementation class* to exist yet, so this went unnoticed until Fiori
Elements tried to render Create and found no implementation to bind to
(RAP's safe failure mode: hide the action, not error). Fixed via ADT's
own quick-fix (**Ctrl+1 on the class name → "Create Behavior
Implementation Class"**), not hand-authored — and the real result
confirms it needed **zero method bodies**: plain managed CRUD with no
custom actions/instance-auth-check needs the class to exist, not to
contain anything.

**Three more real formats learned from this round's Sync** (useful for
everything still ahead):
- **`CLAS`** (any future ABAP class — detector framework, rule engine,
  delta engine, TR generator, doc store adapter): minimal metadata is
  just `<asx:abap version="1.0" .../>`, self-closing, no values at all.
- **`SRVB`** (service binding): root `<SRVB>`, nested `<METADATA>` +
  `<CONTENT>` (bind type/version, `<SERVICES>` list referencing the
  `SRVD` by URI), plus `<PUBLISHED>true</PUBLISHED>` once live. Not
  something to hand-author going forward — it's generated by ADT's
  Publish action, same as the class.
- **`SRVD`**: real shape differs from my `BDEF`-shape guess in a few
  fields (`TYPE` is `SRVD/SRV` not `SRVD/SVD`; adds
  `SRVD_SOURCE_TYPE`/`SRVD_SOURCE_TYPE_DESC`, drops the `LINKS` block) —
  corrected via the Sync, no action needed since it already activated.
- A `G4BA` object and a GUID-named file also appeared — these are
  `/IWFND/V4_ADMIN`'s own Gateway service-group registration and the
  service binding's default-authorization-values object, both entirely
